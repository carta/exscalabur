...
class AppendOnlySheetWriterSpec extends AnyFlatSpec with Matchers {
  ...
  "SheetWriter" should "do substitution from keys in template workbook to output workbook (happy path)" in {
    ...
    val modelSchema: Map[String, YamlEntry] = ExcelTestHelpers.getModelSchema(model)
    ...
    new AppendOnlySheetWriter(templateWorkbook.getSheet(sheetName), outputWorkbook, modelSchema, mutable.Map.empty)
      .writeData(
        staticData = Vector(
          DataCell(s"string_field", "text"),
          DataCell(s"long_field", 1234),
          DataCell(s"double_field", 1234.0)),
        repeatedData = Vector.empty
      )
  ...
  "SheetWriter" should "load template for repeated rows and then insert set of repeated rows" in {
    ...
    new AppendOnlySheetWriter(templateBook.getSheet(templateName), outputWorkbook, schema, mutable.Map.empty)
      .writeData(
