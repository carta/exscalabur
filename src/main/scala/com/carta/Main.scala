...
object Main extends App {
  val sword = new Exscalabur(
    "/home/katie/Desktop/exscalabur.xlsx",
    "/home/katie/Desktop/template.yml"
  )

  val row1 = DataRow.Builder()
    .addCell(DataCell("animal", "dog"))
    .addCell(DataCell("weight", 120))
    .build()
  val row2 = DataRow.Builder()
    .addCell(DataCell("animal", "cat"))
    .addCell(DataCell("weight", 1583812379))
    .build()

  val name = DataCell("name", "katie")
  ...
  sword.addTab(
    "Sheet0",
    "/home/katie/Desktop/template.xlsx",
    List(name),
    List(row1, row2)
  )
